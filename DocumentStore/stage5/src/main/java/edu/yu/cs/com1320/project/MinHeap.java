...
public abstract class MinHeap<E extends Comparable<E>> {
    ...
    protected void downHeap(int k) {
        while (2 * k <= this.count) { //identify which of the 2 children are smaller
            int j = 2 * k;
            if (j < this.count && this.isGreater(j, j + 1)) {
                j++;
            }
            if (!this.isGreater(k, j)) { //if the current value is < the smaller child, we're done
                break;
            }
            this.swap(k, j); //if not, swap and continue testing
            k = j;
        }
    }

    public void insert(E x) {
        if (this.count >= this.elements.length - 1) { // double size of array if necessary
            this.doubleArraySize();
        }
        this.elements[++this.count] = x; //add x to the bottom of the heap
        this.upHeap(this.count); //percolate it up to maintain heap order property
    }
    ...
    public E remove() {
        ...
        this.swap(1, this.count--); //swap root with last, decrement count
        this.downHeap(1); //move new root down as needed
