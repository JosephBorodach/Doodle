...
import java.util.Arrays; //I don't think I need this anymore!
/**
...
public class HashTableImpl<Key, Value> implements HashTable<Key, Value> {
    ...
    public Value get(Key k) {
        ...
        Node<Key,Value> evaluate = (Node<Key,Value>)this.table[hashFunction (k)];
    ...
    private void resize () {
        ...
        for (int i = 0; i < oldSize; i++) {
            Node<Key,Value> current = (Node)temporaryTable[i];
            while (current != null) {
                Key k = current.key;
    ...
    private Value delete(Key k, Value v, Value previousValue) { 
        ...
        Node<Key,Value> evaluate = (Node)this.table[hashFunction(k)];
 ...
 * 7. HashTableImpl and DocumentStoreImpl must both have no-argument constructors.
 ...
 * Avoid collisions using seperate chaning:
 *      Hash table entries point to a linked list of key- value pairs that hashed to it
 *      Make the size of the table (M) sufficiently large that each list stays relatively short
 *      Accordingly, create a linkedlist from scratch using an array in order to use this array based list to aviod collisions with the hashTable array
 */